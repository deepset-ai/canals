...
from typing import Optional, Any, Dict, List, Literal, Union, Tuple
...
class Pipeline:
    ...
    def _state_transition_function(self, current_state: Tuple[Tuple[str, str]]) -> List[str]:
        """
        Given the current state as a list of tuples of (component, socket), returns the next state as list of components
        that must run.
        ...
        for component in self.graph.nodes:
            # If this component has mandatory inputs and received them all all in this state, it can run
            if self.mandatory_input_sockets[component] and set(self.mandatory_input_sockets[component]).issubset(
                set(current_state)
            ):
                transition.append(component)
            # If this component has only optional input sockets, run as soon as there is
            # at least one input socket in the state
            elif not self.mandatory_input_sockets[component] and component in [c for c, _ in current_state]:
                transition.append(component)
    ...
    def run(self, data: Dict[str, Any], debug: bool = False) -> Dict[str, Any]:
        ...
        for component, input_data in data.items():
            for socket, value in asdict(input_data).items():
                if value is not None:
                    next_inputs[(component, socket)] = value
        ...
        # Collect all input socket that have no default and don't come from loops
        self.mandatory_input_sockets = {}
        for component in self.graph.nodes:
            self.mandatory_input_sockets[component] = []
            for socket in self.graph.nodes[component]["input_sockets"]:
                if (
                    not socket in self.graph.nodes[component]["instance"].defaults
                    and not socket in self.graph.nodes[component]["instance"].__canals_optional_inputs__
                ):
                    self.mandatory_input_sockets[component].append((component, socket))
        ...
        while True:
            ...
            # Set the current state and identify the next transition to compute
            current_inputs = deepcopy(next_inputs)
            current_state = tuple(current_inputs.keys())
            current_transition = self._state_transition_function(current_state)
            logger.debug("********")
            logger.debug("%s^ step | State: %s | Transition: %s", step, current_state, current_transition)
            logger.debug("********")
            if len(current_transition) == 0:
                logger.debug("Reached empty transition, stopping.")
                break
            ...
            for component in current_transition:
                ...
                component_inputs = {state[1]: value for state, value in current_inputs.items() if state[0] == component}
                ...
                for used_input in component_inputs.keys():
                    del next_inputs[(component, used_input)]
                ...
                for socket, value in output_values.items():
                    ...
                    if not target_states:
                        if component not in pipeline_output:
                            pipeline_output[component] = {}
                        pipeline_output[component][socket] = value
                        logger.debug(" - '%s.%s' goes to the output with value '%s'", component, socket, value)

                    for target in target_states:
                        if self.is_loop_decision_node(component) and value is None:
                            logger.debug("    Loop decision nodes do not propagate None values.")
                        else:
                            next_inputs[target] = value
                            logger.debug(" - '%s.%s' received '%s'", target[0], target[1], value)

        logger.info("Pipeline executed successfully.")

        # Clean up output from None outputs
        clean_output = {}
    ...
    def is_loop_decision_node(self, component):
        """
        Returns True if the component is a loop decision node.
        """
        loops_back = any(
            networkx.has_path(self.graph, component, self.graph.nodes[component]["input_sockets"][socket].sender)
            for socket in self.graph.nodes[component]["input_sockets"]
        )
        doesnt_loop_back = any(
            not networkx.has_path(self.graph, component, self.graph.nodes[component]["input_sockets"][socket].sender)
            for socket in self.graph.nodes[component]["input_sockets"]
        )
        return loops_back and doesnt_loop_back
    ...
    def _run_component(self, name: str, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if any None was received by a value that was not Optional and if so,
            # return an empty output dataclass ("skipping the component")
